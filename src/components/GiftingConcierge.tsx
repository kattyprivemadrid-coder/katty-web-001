...
const PRESET_QUESTIONS = [
  "¿Cuál es la leyenda detrás del anillo Trinity?",
  "Recomiéndame un reloj clásico como regalo de herencia.",
  "¿Qué significado romántico tiene la colección LOVE?",
  "Quiero sugerencias para un regalo de compromiso de alta gama."
];
...
export default function GiftingConcierge({ onScrollToBoutique }: GiftingConciergeProps) {
  ...
  const [chatHistory, setChatHistory] = useState<ChatMessage[]>([
    {
      id: "initial",
      sender: "concierge",
      text: "Bienvenida a la Maison Katty Privé digital. Es un honor absoluto recibirle. Soy su Consejera Virtual de Regalos de la Maison, a su entera disposición para desvelar el obsequio o la creación icónica que selle un instante memorable en su historia. ¿Desea que exploremos juntas alguna joya o prefiere indicarme sus deseos?",
  ...
  const startConsultation = async () => {
    ...
    const initialMessage = `Hola. Me gustaría que me aconsejes como consejera experta para un regalo de Katty Privé. Mis preferencias iniciales son:\n` +
      `- Destinatario: ${preferences.recipient || "Cualquiera"}\n` +
      `- Ocasión: ${preferences.occasion || "Celebración especial"}\n` +
      `- Categoría: ${preferences.category || "Cualquiera"}\n` +
      `- Presupuesto estimado: ${preferences.budget || "Sin límite"}`;

    const userMessage: ChatMessage = {
      id: Math.random().toString(),
      sender: "user",
      text: `Deseo recibir recomendaciones exclusivas de su consejera para un regalo de Katty Privé: ${preferences.recipient ? "para " + preferences.recipient : "un obsequio especial"}, con motivo de ${preferences.occasion ? preferences.occasion : "una hermosa ocasión"}.`,
      timestamp: new Date().toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" })
    };
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    try {
      ...
      setChatHistory(prev => [...prev, {
        id: Math.random().toString(),
        sender: "concierge",
        text: "Mis disculpas más profundas. Siento no poder asistirle en este preciso instante por una interferencia técnica. Le sugiero ponerse en manos de nuestras expertas agendando una cita privada en boutique.",
        timestamp: new Date().toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" })
      }]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <section id="gifting-concierge" className="bg-[#120002] text-[#FAF9F6] py-20 px-6 border-t border-b border-[#c5a880]/30 relative overflow-hidden">
      
      {/* Background radial shimmer */}
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-[#8c1d27]/10 rounded-full blur-[120px] pointer-events-none" />
          ...
          <div className="space-y-4">
            <div className="flex items-center space-x-2">
              ...
              <span className="font-mono text-xs tracking-[0.3em] text-[#c5a880] uppercase">
                Servicio Exclusivo Katty Privé
              </span>
            </div>
            <h3 className="font-serif text-3xl md:text-5xl font-light tracking-wide leading-tight">
              Consejería de Obsequios
            </h3>
            <p className="font-sans text-xs md:text-sm font-light text-[#FAF9F6]/70 leading-relaxed tracking-wide">
              Un ritual de búsqueda guiado por la elegancia de la Maison. Indique sus intenciones o converse directamente con nuestra Consejera Virtual de Inteligencia Artificial para esculpir la elección impecable.
            </p>
          </div>
              ...
              >
                <div className="flex items-center gap-2 border-b border-[#c5a880]/15 pb-3">
                  ...
                  <h4 className="font-serif text-sm tracking-widest uppercase text-[#FAF9F6]">
                    Planificador de Intenciones
                  </h4>
                </div>

                {/* Destination / Recipient selection */}
                <div className="space-y-1.5">
                  <label className="text-[10px] tracking-widest uppercase text-[#FAF9F6]/60">¿Para quién es el regalo?</label>
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    {["Para Ella", "Para Él", "Para Pareja"].map((val) => (
                      <button
                        key={val}
                ...
                {/* Occasion Selection */}
                <div className="space-y-1.5">
                  <label className="text-[10px] tracking-widest uppercase text-[#FAF9F6]/60">¿Cuál es la ocasión?</label>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    {["Aniversario", "Compromiso", "Cumpleaños", "Un Capricho"].map((val) => (
                      <button
                        key={val}
        ...
        <div className="lg:col-span-7 flex flex-col h-[520px] bg-black/40 border border-[#c5a880]/20 rounded-sm overflow-hidden shadow-2xl relative">
          ...
          <div className="p-4 bg-black/60 border-b border-[#c5a880]/15 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h4 className="font-serif text-sm font-normal text-[#FAF9F6] tracking-wide">Consejera Virtual de Regalos</h4>
                <p className="text-[10px] font-light text-emerald-400">En línea • Consejera Especialista Katty Privé</p>
              </div>
            </div>
              ...
              >
                <div
                  className={`max-w-[85%] rounded-sm p-4 text-xs leading-relaxed space-y-1.5 ${
                    msg.sender === "user"
                      ? "bg-[#8c1d27]/25 text-[#FAF9F6] border border-[#8c1d27]/40"
                ...
                >
                  {msg.sender === "concierge" && (
                    <div className="flex items-center space-x-1 mb-1">
                       ...
                       <span className="text-[9px] uppercase tracking-widest text-[#c5a880] font-semibold">Katty Privé</span>
                    </div>
                  )}
