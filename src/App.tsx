...
export default function App() {
  ...
  const [customProductsMap, setCustomProductsMap] = useState<Record<string, Product>>(() => {
    try {
      const saved = localStorage.getItem("kattyprive_custom_products");
      if (!saved) return {};
      const parsed = JSON.parse(saved);
      const cleaned: Record<string, Product> = {};
      for (const [id, item] of Object.entries(parsed as Record<string, Product>)) {
        if (id === "bags-06" && item.name?.includes("Lady Katty")) {
          continue;
        }
        if (id === "bags-03" && item.name?.includes("Guess")) {
          continue;
        }
        if (id === "bags-01" && (item.name?.includes("Michael Kors") || item.name?.includes("Guess"))) {
          continue;
        }
        ...
        if (id.startsWith("bags-") && (
          ...
          item.name?.includes("Baguette 1990") ||
          item.name?.includes("Embajador") ||
          item.name?.includes("Hobo") ||
          item.name?.includes("Trinity") ||
          item.name?.includes("Vanity Case") ||
          item.name?.includes("Piramidal") ||
          item.name?.includes("Weekender") ||
          item.image?.includes("unsplash.com")
        )) {
          continue;
        }
        const defaultProd = PRODUCTS_DATA.find((p) => p.id === id);
        if (defaultProd) {
          const itemCopy = { ...item };
          // If stored image is an old unsplash image and default has genuine local photo, prioritize default
          if (itemCopy.image && itemCopy.image.includes("unsplash.com") && defaultProd.image && !defaultProd.image.includes("unsplash.com")) {
            itemCopy.image = defaultProd.image;
            itemCopy.name = defaultProd.name;
            itemCopy.description = defaultProd.description;
            itemCopy.price = defaultProd.price;
            itemCopy.details = defaultProd.details;
            itemCopy.history = defaultProd.history;
          } else if (defaultProd.category === "cosmetics" && itemCopy.image && itemCopy.image.includes("unsplash.com")) {
            itemCopy.image = defaultProd.image;
            itemCopy.name = defaultProd.name;
            itemCopy.description = defaultProd.description;
            itemCopy.price = defaultProd.price;
      ...
      try {
        ...
        for (const [id, item] of Object.entries(parsed as Record<string, Product>)) {
          ...
          if (id.startsWith("cosmetics-") && (
            item.name?.includes("Esmalte de Uñas") ||
            item.name?.includes("Bruma Hidratante") ||
            item.name?.includes("Cofre de Belleza 'Ritual Supremo'") ||
            item.name?.includes("Exfoliante Facial Suave") ||
            item.name?.includes("Loción Tónica Facial Equilibrante") ||
            item.name?.includes("Mascarilla Nocturna") ||
            item.name?.includes("Leche Limpiadora Facial") ||
            item.name?.includes("Mascarilla Capilar Nutrición Intensa") ||
            item.name?.includes("Suero Reparador de Puntas") ||
            item.name?.includes("Bálsamo Corporal Hidratante") ||
            item.name?.includes("Jabón Natural Artesanal de Glicerina") ||
            item.image?.includes("unsplash.com")
          )) {
            continue;
          }
  ...
  const [cart, setCart] = useState<CartItem[]>(() => {
    try {
      const saved = localStorage.getItem("kattyprive_cart");
      if (!saved) return [];
  ...
  const [wishlist, setWishlist] = useState<Product[]>(() => {
    try {
      const saved = localStorage.getItem("kattyprive_wishlist");
      if (!saved) return [];
  ...
  const [appointments, setAppointments] = useState<Appointment[]>(() => {
    const saved = localStorage.getItem("kattyprive_appointments");
    return saved ? JSON.parse(saved) : [];
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem("kattyprive_custom_products", JSON.stringify(customProductsMap));
    } catch (err) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("kattyprive_sold_out_ids", JSON.stringify(soldOutProductIds));
    } catch (err) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("kattyprive_cart", JSON.stringify(cart));
    } catch (err) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("kattyprive_wishlist", JSON.stringify(wishlist));
    } catch (err) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("kattyprive_appointments", JSON.stringify(appointments));
    } catch (err) {
  ...
  const handleCompletePurchase = (customer: OrderCustomerDetails, items: CartItem[]) => {
    ...
    const message = `🛍️ *NUEVO PEDIDO FORMALIZADO - MAISON KATTY PRIVÉ* 🛍️
...
⚠️ _Las piezas compradas han pasado automáticamente a estado NO DISPONIBLE en el catálogo de la Maison._

_Notificación enviada desde la web oficial: https://kattyprivemadrid.netlify.app/_`;

    const waUrl = `https://wa.me/34632892657?text=${encodeURIComponent(message)}`;
  ...
  const handleUpdateProduct = (updatedProduct: Product) => {
    ...
    setCustomProductsMap((prev) => {
      ...
      try {
        localStorage.setItem("kattyprive_custom_products", JSON.stringify(next));
      } catch (err) {
    ...
    return (
      <div className="h-screen w-screen bg-[#FAF9F6] flex flex-col items-center justify-center space-y-8">
        ...
        <div className="flex items-center space-x-2 text-xs font-mono text-[#c5a880] tracking-widest uppercase">
          <Sparkles size={14} className="animate-spin text-[#8c1d27]" />
          <span>Iniciando Maison Katty Privé...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between selection:bg-[#8c1d27]/20 selection:text-[#120002] relative">
      {/* Barra Privada de Dirección (Solo cuando la dueña ha iniciado sesión) */}
      {isAdmin && (
        <div className="bg-[#120002] text-[#FAF9F6] px-4 py-2 text-xs font-mono border-b border-[#c5a880]/40 flex items-center justify-between z-50 sticky top-0 shadow-md">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-[#c5a880] font-semibold uppercase tracking-wider text-[10px]">
              Modo Privado Dueña de Boutique Activo
            </span>
            <span className="text-white/60 hidden md:inline text-[10px]">
      ...
      <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-2">
        <a
          href="https://wa.me/34632892657?text=Hola,%20deseo%20asesoramiento%20personalizado%20sobre%20las%20piezas%20de%20Katty%20Priv%C3%A9"
          target="_blank"
          rel="noopener noreferrer"
          className="group flex items-center gap-2.5 bg-[#120002] hover:bg-[#8c1d27] text-white pl-4 pr-3 py-2.5 rounded-full shadow-2xl border border-[#c5a880]/50 transition-all duration-300 hover:scale-105"
          title="Atención inmediata WhatsApp Katty Privé"
        >
