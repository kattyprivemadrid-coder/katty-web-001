...
export default function Catalog({ 
  ...
  const getCategorySpanishLabel = (cat: ProductCategory) => {
    switch (cat) {
      ...
      case "cosmetics": return "Cosmética";
      case "clothing": return "Ropa";
      case "jewelry": return "Bisutería";
      case "bags": return "Bolsos";
      default: return cat;
    }
  };
  ...
  return (
    <section id="catalog" className="max-w-7xl mx-auto px-4 sm:px-6 py-16 scroll-mt-28">
      ...
      <div className="text-center space-y-3 mb-10">
        <span className="font-mono text-[11px] tracking-[0.35em] text-[#8c1d27] uppercase">
          La Maison Katty Privé
        </span>
        <h3 className="font-serif text-3xl md:text-5xl font-light tracking-wider text-[#120002]">
          Colecciones Icónicas
        </h3>
        <p className="font-sans text-xs md:text-sm font-light text-[#121212]/50 max-w-xl mx-auto tracking-widest uppercase">
          Diseños atemporales cincelados con precisión histórica
        </p>
          ...
          >
            ...
            <div className="relative z-10 p-6 sm:p-10 md:p-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="space-y-3 max-w-2xl">
                ...
                <div className="flex flex-wrap items-center gap-4 pt-2 text-[10px] font-mono text-[#c5a880]">
                  <span className="flex items-center gap-1.5">
                    <Sparkles size={12} />
                    15 Creaciones Exclusivas
                  </span>
                  <span className="text-[#c5a880]/40">•</span>
                  <span className="flex items-center gap-1.5">
                    <ShieldCheck size={12} />
                    Certificado de Autenticidad Katty Privé
                  </span>
                  <span className="text-[#c5a880]/40">•</span>
                  <span className="flex items-center gap-1.5">
                    <PackageCheck size={12} />
                    Envío de Cortesía Asegurado
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row md:flex-col gap-3">
                <button
                  onClick={() => setSelectedCategory("all")}
                  className="px-5 py-3 border border-[#c5a880]/40 hover:border-[#FAF9F6] bg-[#120002]/60 hover:bg-[#FAF9F6] text-[#FAF9F6] hover:text-[#120002] text-[11px] tracking-[0.2em] uppercase font-light transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer backdrop-blur-sm"
                >
                  <ArrowLeft size={13} />
        ...
        <div className="mb-14">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <h4 className="font-serif text-xl sm:text-2xl font-light text-[#120002]">
                Explorar Secciones de la Maison
              </h4>
            </div>
            <p className="text-xs text-[#121212]/50 font-light hidden sm:block">
              Haga clic en una sección para ver sus 15 artículos
            </p>
          </div>
            ...
            {CATEGORIES_CONFIG.map((cat) => {
              ...
              return (
                <button
                  ...
                  className="group relative overflow-hidden aspect-[4/5] rounded-sm border border-[#c5a880]/20 hover:border-[#8c1d27] shadow-sm hover:shadow-lg transition-all duration-500 text-left p-3 flex flex-col justify-between cursor-pointer"
                >
                  <img
                    src={cat.bannerImage}
                    alt={cat.label}
                  ...
                  <div className="relative z-10 flex justify-between items-start">
                    <span className="text-[9px] font-mono text-[#c5a880] bg-[#120002]/80 px-2 py-0.5 rounded-full border border-[#c5a880]/30 backdrop-blur-xs">
                      {count} piezas
                    </span>
                  </div>
                  ...
                  <div className="relative z-10 space-y-1">
                    ...
                    <p className="text-[10px] text-[#FAF9F6]/70 line-clamp-1 font-light">
                      {cat.tagline}
                    </p>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 mb-8 border-b border-[#c5a880]/15">
        <div className="flex items-center gap-3">
          <span className="font-serif text-lg font-light text-[#120002]">
            {selectedCategory === "all" ? "Todas las Creaciones" : `Creaciones de ${currentCategoryInfo?.label}`}
          </span>
          <span className="text-xs font-mono text-[#8c1d27] bg-[#8c1d27]/10 px-2.5 py-0.5 rounded-full font-medium">
                ...
                <div className="relative overflow-hidden aspect-square bg-[#FBFBFA]">
                  ...
                  {isSoldOut ? (
                    ...
                    <div className="absolute inset-0 bg-black/30 flex items-center justify-center p-4 z-10 text-center pointer-events-none">
                      <div className="bg-[#120002]/90 backdrop-blur-[2px] border border-amber-500/40 px-3.5 py-1.5 rounded-xs shadow-md">
                  ...
                  >
                    <Heart
                      ...
                      className={`transition-colors ${
                        isInWishlist(product.id) ? "fill-[#8c1d27] text-[#8c1d27]" : "text-stone-600"
                      }`}
                    />
                  </button>
                ...
                <div className="p-6 text-center space-y-2.5 bg-[#FAF9F6]">
                  <div className="flex items-center justify-center gap-2">
                    <button 
                      ...
                      className="font-mono text-[9px] tracking-[0.25em] text-[#c5a880] hover:text-[#8c1d27] transition-colors uppercase cursor-pointer"
                      title={`Ver todos los artículos de ${getCategorySpanishLabel(product.category)}`}
                    >
                      {product.category === "perfumes" && "Perfumes • Parfumerie"}
                      {product.category === "glasses" && "Gafas • Haute Lunetterie"}
                      {product.category === "cosmetics" && "Cosmética • Soins Précieux"}
                      {product.category === "clothing" && "Ropa • Haute Couture"}
                      {product.category === "jewelry" && "Bisutería & Alta Joyería"}
                      {product.category === "bags" && "Bolsos • Maroquinerie d'Art"}
                    </button>
                  </div>
                  ...
                  <h4 
                    ...
                    className="font-serif text-lg font-normal text-[#120002] tracking-wide truncate hover:text-[#8c1d27] transition-colors cursor-pointer"
                    title={product.name}
                  >
                    {product.name}
                  </h4>
                  
                  <p className="font-sans text-xs font-light text-[#121212]/60 line-clamp-2 h-10 px-2 leading-relaxed">
                    {product.description}
                  </p>
                    ...
                    {isAdmin && (
                      <div className="flex items-center gap-1.5 ml-1 pt-1">
                        <button
                          ...
                          className="px-2 py-0.5 bg-[#c5a880]/20 hover:bg-[#8c1d27] text-[#8c1d27] hover:text-white border border-[#c5a880]/60 rounded-xs text-[10px] font-mono uppercase tracking-wider flex items-center gap-1 transition-all cursor-pointer shadow-2xs"
                        ...
                        {onUpdateProduct && (
                          <button
                            ...
                            className="p-1 text-stone-500 hover:text-white hover:bg-[#120002] bg-white border border-stone-300 transition-colors rounded-xs cursor-pointer shadow-2xs"
                  ...
                  <div className="pt-4">
                    {status === "sold_out" ? (
                      ...
                      <button
                        ...
                        className="w-full bg-[#120002] hover:bg-[#8c1d27] text-white py-3 text-[11px] font-light tracking-[0.2em] uppercase transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer shadow-sm rounded-xs"
                      >
                        <ShoppingBag size={13} />
                        Añadir a la Bolsa
                      </button>
              ...
              >
                ...
                <div className="md:w-1/2 h-full overflow-y-auto p-6 md:p-10 flex flex-col justify-between">
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <div className="flex items-center space-x-2">
                        ...
                        <button
                          ...
                          className="font-mono text-[10px] tracking-[0.2em] text-[#8c1d27] hover:underline uppercase cursor-pointer"
                        >
                      ...
                      <div className="flex items-center justify-between gap-4">
                        ...
                        {onUpdateProduct && (
                          <button
                            ...
                            className="flex items-center gap-1.5 text-xs font-mono text-stone-500 hover:text-[#8c1d27] border border-stone-300 hover:border-[#8c1d27] px-2.5 py-1.5 rounded transition-colors uppercase tracking-wider shrink-0 cursor-pointer"
                    ...
                    {/* Description */}
                    <p className="text-xs font-light text-[#121212]/75 leading-relaxed tracking-wide font-sans">
                    ...
                    <div className="space-y-4">
                      <div>
                        <h5 className="font-serif text-xs font-semibold tracking-wider text-[#120002] uppercase mb-2">
                          Detalles de la Creación
                        </h5>
                        <ul className="text-[11px] font-light text-[#121212]/70 space-y-1.5 list-disc pl-4 tracking-wide">
                      ...
                      <div className="pt-2">
                        <h5 className="font-serif text-xs font-semibold tracking-wider text-[#8c1d27] uppercase mb-1">
                          Saber Hacer & Historia
                        </h5>
                        <p className="text-[11px] italic font-light text-[#121212]/70 leading-relaxed bg-[#c5a880]/5 p-3 border-l-2 border-[#c5a880]">
                          ...
                          >
                            <div className="space-y-3">
                              <p className="text-[10px] text-[#121212]/60 leading-relaxed">
                                Ajuste el diámetro o circunferencia para verificar la medida recomendada por los orfebres de Katty Privé:
                              </p>
                  ...
                  <div className="mt-8 pt-6 border-t border-[#c5a880]/15 space-y-4">
                    ...
                    <div className="flex gap-4">
                      {qvStatus === "sold_out" ? (
                        ...
                        <button
                          disabled
                          className="flex-1 bg-amber-50 text-amber-800 py-4 text-xs font-mono font-bold tracking-[0.2em] uppercase flex items-center justify-center gap-2 cursor-not-allowed border border-amber-300 rounded-xs"
                        >
                          <Clock size={14} />
                        ...
                        <button
                          onClick={() => {
                            if (currentQuickViewProduct.category === "clothing") {
                              addToCart({ ...currentQuickViewProduct, selectedSize: selectedClothingSize });
                            } else {
                              addToCart(currentQuickViewProduct);
                            }
                            setQuickViewProduct(null);
                          }}
                          className="flex-1 bg-[#120002] hover:bg-[#8c1d27] text-white py-4 text-xs font-light tracking-[0.25em] uppercase transition-colors flex items-center justify-center gap-2 cursor-pointer shadow rounded-xs"
                        >
                          <ShoppingBag size={14} />
                          Añadir a la Bolsa {currentQuickViewProduct.category === "clothing" && `(Talla ${selectedClothingSize})`}
                        </button>
                      )}
                    </div>
                  </div>
