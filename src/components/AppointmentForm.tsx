...
const MODALITY_OPTIONS = [
  { id: "domicilio", name: "Visita a su domicilio", label: "Visita a su domicilio (Atención Privada)" },
  { id: "whatsapp", name: "Asistencia personalizada vía WhatsApp", label: "Asistencia personalizada vía WhatsApp (+34 632 89 26 57)" }
];

const SERVICES = [
  "Perfumes (Parfumerie d'Art)",
...
export default function AppointmentForm({ onAddAppointment }: AppointmentFormProps) {
  ...
  const generateWhatsAppUrl = (appt: Appointment, isModification = false) => {
    const header = isModification
      ? "⚠️ *AVISO DE MODIFICACIÓN DE CITA - MAISON KATTY PRIVÉ*"
      : "✨ *NUEVA RESERVA DE CITA PRIVADA - MAISON KATTY PRIVÉ*";
...
📅 *Fecha:* ${appt.date}
⏰ *Hora:* ${appt.time} hs
${appt.notes ? `📝 *Notas / Dirección:* ${appt.notes}` : ""}

_Notificación enviada desde la web oficial: https://kattyprivemadrid.netlify.app/_`;

    return `https://wa.me/34632892657?text=${encodeURIComponent(message)}`;
  };
  ...
  return (
    <section id="boutiques" className="max-w-4xl mx-auto px-6 py-20">
      ...
      <div className="text-center space-y-4 mb-14">
        ...
        <span className="font-mono text-xs tracking-[0.3em] text-[#8c1d27] uppercase">
          Encuentros en Persona
        </span>
        <h3 className="font-serif text-3xl md:text-5xl font-light tracking-wider text-[#120002]">
          Reserve su Cita Personal
        </h3>
        <p className="font-sans text-xs md:text-sm font-light text-[#121212]/70 max-w-xl mx-auto tracking-widest uppercase">
          SIENTA LA EXCELENCIA DEL TRATO PRIVADO DE NUESTRAS EXCLUSIVAS DE KATTY PRIVÉ
        </p>
      ...
      <AnimatePresence mode="wait">
        {!bookingCompleted ? (
          ...
          >
            ...
            <div className="md:col-span-5 bg-[#120002] text-[#FAF9F6] p-8 md:p-10 flex flex-col justify-between space-y-8 relative">
              <div className="absolute inset-0 bg-cover bg-center opacity-10" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1547996160-81dfa63595aa?auto=format&fit=crop&q=80&w=600')" }} />
              
              <div className="space-y-6 relative z-10">
                <h4 className="font-serif text-2xl font-light text-[#c5a880] tracking-wide">
                  La Experiencia Katty Privé
                </h4>
                <p className="text-xs font-light text-[#FAF9F6]/80 leading-relaxed tracking-wider">
                  Le aguarda una visita de cortesía dirigida por nuestras especialistas más cualificadas. Analizaremos en conjunto el legado de la pieza de su interés con una atención dedicada exclusiva
                </p>
                ...
                <div className="space-y-4 pt-4 border-t border-[#c5a880]/20 text-xs font-light">
                  ...
                  <div className="flex items-start gap-3 pt-2 border-t border-[#c5a880]/15">
                    ...
                    <div>
                      ...
                      <a href="https://wa.me/34632892657" target="_blank" rel="noopener noreferrer" className="text-[11px] text-[#FAF9F6]/90 hover:text-[#c5a880] transition-colors mt-0.5 block font-mono">
                        +34 632 89 26 57
                      </a>
                    </div>
                  </div>
                ...
                {isWhatsAppSelected && (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-mono text-[#128C7E] font-bold">+34 632 89 26 57</span>
                    </div>
                    <p className="text-[10px] text-[#121212]/70 leading-relaxed font-light">
                      Puede iniciar la conversación directamente haciendo clic en el botón inferior o completar el formulario para agendar una sesión en fecha y hora específica.
                    </p>
                    <a
                      href="https://wa.me/34632892657?text=Hola%2C%20solicito%20asistencia%20personalizada%20con%20la%20Maison%20Katty%20Priv%C3%A9"
                      target="_blank"
                      rel="noopener noreferrer"
                    ...
                    >
                      ...
                      Abrir WhatsApp (+34 632 89 26 57)
                    </a>
                  </motion.div>
                )}
          ...
          >
            {/* Elegant watermark */}
            <div className="absolute -right-12 -bottom-12 w-48 h-48 rounded-full border border-[#c5a880]/10 flex items-center justify-center text-stone-100 font-serif text-9xl">K</div>

            <div className="w-16 h-16 bg-[#c5a880]/10 border border-[#c5a880] rounded-full flex items-center justify-center mx-auto text-[#8c1d27]">
              <CheckCircle size={32} />
            </div>

            <div className="space-y-2">
              <span className="font-mono text-[9px] tracking-[0.3em] text-[#c5a880] uppercase">Reserva Formalizada</span>
              <h4 className="font-serif text-3xl font-light text-[#120002]">¡Su cita ha sido Agendada!</h4>
              <p className="text-xs font-light text-[#121212]/60 max-w-md mx-auto leading-relaxed">
                Le agradecemos su confianza en la Maison Katty Privé. Hemos reservado una suite de atención privada para usted. Recibirá un folleto de confirmación digital con los detalles de etiqueta en su correo electrónico.
              </p>
            </div>
            ...
            <div className="bg-[#FAF9F6] border border-[#c5a880]/20 p-4 text-left text-xs space-y-3 font-light tracking-wide rounded-sm">
              <div className="flex justify-between border-b border-[#c5a880]/15 pb-2 text-[10px] tracking-widest text-[#121212]/50 uppercase font-semibold">
                <span>Resumen del Encuentro</span>
                <span className="text-[#8c1d27]">Maison Privée</span>
              </div>
              <div>
